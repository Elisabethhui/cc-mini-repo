...
#add
def should_runtime_compact(
    messages: list[dict],
    model: str | None = None,
    last_input_tokens: int | None = None,
) -> bool:
    """
    Stricter runtime compact trigger for smaller local models.
    """
    return should_compact(messages, model=model, last_input_tokens=last_input_tokens)
#
# ---------------------------------------------------------------------------
# Message splitting
# ---------------------------------------------------------------------------
...
class CompactService:
    ...
    def compact_messages(
        ...
        """
        Runtime-safe wrapper around compact().
        Returns only the new message list.
        """
        new_messages, _summary = self.compact(
            messages=messages,
            system_prompt=system_prompt,
            custom_instructions=custom_instructions or reason,
        )
        return new_messages
# ---------------------------------------------------------------------------
# Media stripping
