...
Add a delivery-closure phase that turns a completed coding task into a verifiable, reviewable, commit-ready, and closeable unit of work. The objective is to connect the existing coding workflow to the project-level milestone flow without merging the two concerns into one large subsystem.
...
What is still missing is the end-of-work product loop: after a task is implemented, the system should help the user verify it, review it, commit it, and capture a concise closeout summary that can be reused for milestone audit and later release notes.
...
The command layer is responsible for execution order and user-facing prompts. The milestone layer is responsible for capturing evidence and recording what was completed. They should communicate through structured outputs, not shared mutable state.
...
- Commit turns a validated result into a durable git artifact
- Closeout summarizes what shipped and what remains deferred

This should be expressed as a small, bounded flow rather than a monolithic "ship everything" command. The existing `/review`, `/test`, and `/commit` skills already cover parts of the loop; the new work should make the sequence obvious and repeatable.
...
5. If review passes, the change is committed.
6. A closeout summary is emitted.
7. The closeout summary is used for milestone audit and any later release-note or archive work.
